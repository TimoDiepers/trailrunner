...
```
    ...
    1 t Portland cement, aluminous cement, slag cement and similar hydraulic cements, except in the form of clinkers @DK/2030-06-15  [model: CementPlant; unit: t -> kg ×1000]
      ...
      2475 MJ Natural gas, liquefied or in the gaseous state @DK/2030-06-15 (https://vocab.sentier.dev/units/quantity-kind/Pressure=400000 Pa)  [cutoff: coverage_excluded]
...
400000 Pa (4 bar), as the quantity kind
`https://vocab.sentier.dev/units/quantity-kind/Pressure`, and `NaturalGasSupply` declares in its
...
demand can ask for. Conditions are named by IRI, here from the
[sentier vocabulary](https://vocab.sentier.dev) (derived from QUDT), so a supplier
matches only if it means the same pressure in the same unit. This plant's burners
take gas at 4e5 Pa (4 bar), so its gas demand carries
`https://vocab.sentier.dev/units/quantity-kind/Pressure` = 400000 Pa in its [`Flow`](api/flow.md)'s `context`. `NaturalGasSupply`
...
class CementKilnConstruction(Model):
    ...
    steel_per_capacity = 12.0       # kg steel per t/yr of clinker capacity
    aluminium_per_capacity = 0.8    # kg aluminium, likewise
...
PROXY = ProxySettings(context_tolerance={natural_gas.PRESSURE: (0.0, 1e5, PA)})
