...
## 6. Make the run's choices explicit
...
## 7. Bound the walk: `--max-depth`, `--max-nodes`
...
- **Only the model tier.** The CLI builds a plain `Glossary`, so a demand no model answers
  is always a cutoff. Generalising a demand along the vocabulary, or borrowing from a
