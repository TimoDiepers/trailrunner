...
are deliberate: the traversal never knows how a process works, and a process never
...
- two or more → [`AmbiguousModelMatch`](../api/errors.md); two models producing the same flow
...
- **Unresolvable data** becomes a recorded leaf in the log — `no_model_found`,
