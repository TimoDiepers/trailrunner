...
A model is Python code for exactly one technology. It never sees the rest of the supply
...
`coverage_excluded` rather than `no_producer`, with a `detail` naming the model — so the
...
default year, model-specific switches. Anything varying per technology belongs in that
technology's parameter set instead.
...
[`AmbiguousProducer`](../api/errors.md). Narrow one of their coverages.
